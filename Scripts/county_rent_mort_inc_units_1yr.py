...
county_vacancy_df = pd.read_csv('county_vacancy_raw_1yr_1018.csv')  # Total units vacant
county_hh_df = pd.read_csv('irs_mig_91to2018.csv')  # Extracting county household count
county_pop_df = pd.read_csv('county_pops_1yr_1018.csv')  # Extracting county population count

master_df = pd.DataFrame()
metadata = os.listdir('Counties 1yr')[1::3]
counter = 0
for file in os.listdir('Counties 1yr')[::3]:
    year = int(file[7:11])
    print(year, file)
    data_df = pd.read_csv('Counties 1yr/' + file)[1:]
    meta = pd.read_csv('Counties 1yr/' + metadata[counter])[1:]
    ...
    dataframe = pd.DataFrame({'Year':list(np.full(len(owners), year)), 'FIPS':data_df.FIPS.unique(), 'County':data_df.County.unique(),
    ...
    hh_df = county_hh_df[county_hh_df.Year == year]
    hh_dict = {}
    ...
    pop_df = county_pop_df[county_pop_df.Year == year]
    pop_dict = {}
    ...
    vac_df = county_vacancy_df[county_vacancy_df.Year == year]
    owned_dict = {}
    rent_dict = {}
...
master_df[master_df.Year == 2018].to_csv('county_rent_mort_inc_units_1yr_2018' + '.csv', index=False)
master_df.to_csv('county_rent_mort_inc_units_1yr.csv', index=False)