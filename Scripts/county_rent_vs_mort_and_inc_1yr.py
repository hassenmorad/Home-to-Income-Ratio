...
for file in os.listdir('Counties 1yr')[::3]:
    ...
    dataframe = dataframe[~((dataframe.Rent_Share_Inc_dec > .5) & (dataframe.Total_Units < 10000))]  # Excluding outliers (a few low pop counties from each year)
    
    dataframe.to_csv('county_rent_vs_mort_and_inc_1yr_' + str(year) + '.csv', index=False)
    
    master_df = pd.concat([master_df, dataframe])
    
    counter += 1
    
master_df.to_csv('county_rent_vs_mort_and_inc_1yr.csv', index=False)