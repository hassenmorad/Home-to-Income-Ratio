...
metadata = os.listdir('US 5yr')[1::3]
counter = 0
for file in os.listdir('US 5yr')[::3]:
    year = int(file[7:11])
    print(year, file)
    data_df = pd.read_csv('US 5yr/' + file)[1:]
    meta = pd.read_csv('US 5yr/' + metadata[counter])[1:]
    ...
    dataframe.to_csv('national_rent_vs_mort_and_inc_5yr_' + str(year-2) + '.csv', index=False)
    
    master_df = pd.concat([master_df, dataframe])
    counter += 1
    
master_df.to_csv('national_rent_vs_mort_and_inc_5yr_0816.csv', index=False)