...
metadata = os.listdir('Regions 1yr')[1::3]
counter = 0
for file in os.listdir('Regions 1yr')[::3]:
    year = int(file[7:11])
    print(year, file)
    data_df = pd.read_csv('Regions 1yr/' + file)[1:]
    meta = pd.read_csv('Regions 1yr/' + metadata[counter])[1:]
    ...
    dataframe.to_csv('region_rent_vs_mort_and_inc_1yr_' + str(year) + '.csv', index=False)
    
    master_df = pd.concat([master_df, dataframe])
    counter += 1
    
master_df.to_csv('region_rent_vs_mort_and_inc_1yr.csv', index=False)