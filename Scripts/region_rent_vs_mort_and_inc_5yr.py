...
metadata = os.listdir('Regions 5yr')[1::3]
counter = 0
for file in os.listdir('Regions 5yr')[::3]:
    year = int(file[7:11])
    print(year, file)
    data_df = pd.read_csv('Regions 5yr/' + file)[1:]
    meta = pd.read_csv('Regions 5yr/' + metadata[counter])[1:]
    ...
    for i,row in data_df.iterrows():
        if 'N' in row.values or '-' in row.values or '100-' in row.values or '(X)' in row.values:
            drop.append(i)
    print('regions excluded:', len(drop))        
    ...
    dataframe = pd.DataFrame({'Year':list(np.full(len(owners), year-2)), 'Region':data_df.Region.unique(), 'Owners30':owners, 'Renters30':renters, 
    ...
    dataframe.to_csv('region_rent_vs_mort_and_inc_5yr_' + str(year) + '.csv', index=False)
    
    master_df = pd.concat([master_df, dataframe])
    counter += 1
    
master_df.to_csv('region_rent_vs_mort_and_inc_5yr.csv', index=False)