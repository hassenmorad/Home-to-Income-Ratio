...
for year in range(2008, 2017):
    ...
    for fips_group in fips_groups:
        ...
        for fips in fips_group:
            med_rent = yr_df.Med_Rent[yr_df.FIPS == fips].iloc[0]
...
df.to_csv('rent_by_pop_den_5yr_0816.csv', index=False)

"""med_rent = []
for col in df.columns:
    med_rent += list(df[col].values)
    
types = []
for col in df.columns:
    types += [col]*9  # Number of years
    
pop_den_df = pd.DataFrame({'Year':list(range(2008, 2017))*6, 'Med_Rent':med_rent, 'Home_Units':types})

pop_den_df.to_csv('rent_by_pop_den_5yr_0816.csv', index=False)"""