...
types = []
for col in df.columns:
    types += [col]*9  # Number of years included
    
pop_den_df = pd.DataFrame({'Year':list(range(2010, 2019))*6, 'Rent_Share_Inc':rent_share, 'Home_Units':types})

pop_den_df.to_csv('rent_share_inc_by_hous_den_1yr_1018.csv', index=False)